...
3. In OBS: Docks → Custom Browser Docks → add the dock URL (a `file://` URL shown in the README and copyable from `setup.html` — a static helper page built into `dist/` beside the dock and overlay pages; opened by double-clicking it in any browser, it derives the correct absolute dock URL and the password-free overlay source URL from its own location).
...
5. In OBS: add a Browser Source pointing at `overlay.html` — the dock's Diagnostics view and `setup.html` state the exact recommended settings: width/height = canvas resolution, FPS 30 (or 60 to match the project), **"Shutdown source when not visible" unchecked**, **"Refresh browser when scene becomes active" unchecked**. When connected, the diagnostic compares the actual Browser Source settings against these recommendations and names each mismatch with its expected value.
...
- **LIVE detection layers:** primary — the overlay's `window.obsstudio` `obsSourceActiveChanged` / `obsSourceVisibleChanged` events relayed to the dock (zero configuration); secondary — obs-websocket `GetSourceActive.videoActive`, with the client OR-ing the high-volume `InputActiveStateChanged` event flag (1<<17) into its subscription and re-polling on reconnect. "In Preview" is best-effort and labelled `SHOWING (PREVIEW)` (derived from `videoShowing`); no recursive scene-walk in MVP.
- The Live view always shows a state chip — **LIVE / SHOWING / HIDDEN / UNKNOWN** — above the count controls.
- **Chip precedence (locked for Phase 3):** each detection layer reports `active`/`showing` as true / false / unknown, and is trusted only while fresh (overlay relay — while the overlay's heartbeat is current on any transport; websocket — while identified, re-polled on identify and periodically). Merged `active`/`showing` is the OR of trusted layers. The chip reads: **LIVE** when the source is active and the content is render-shown; **HIDDEN** with a "source is live in Program" detail when the source is active but the operator has render-hidden the content (the audience currently sees nothing — the detail plus the armed warning carry the danger); **SHOWING (Preview)** when `videoShowing` without active; plain **SHOWING/HIDDEN** from the render-level flag when no layer reports activity but an overlay page has been seen recently; **UNKNOWN** only when neither an overlay page nor a websocket connection provides any basis.
- **Warning condition:** Show overlay (and Reset/Jump) warn when the mapped source is **active in Program** — not merely when Studio Mode is off (a source shown into the current Program scene bypasses transitions even with Studio Mode on). Studio-Mode-off remains a secondary warning case (it arms the warning when source activity is unknown). The blocking confirmation appears once per session with "don't ask again this session"; the ambient chip is the continuous safeguard. Bridge-hotkey commands bypass the blocking confirmation — the operator triggering a global hotkey may not be looking at the dock, and a modal would make the hotkey appear dead; the chip and the event log remain the safeguard for hotkey-sourced changes.
...
- Pause/Resume maps to the automatic timer only: the dock resolves the toggle against the current status (`running` → pause, `paused` → resume); in a manual-mode session, or with no session, the bridge command is rejected as invalid and logged (§8.13) — it never switches modes. Show/Hide is likewise a toggle the dock resolves against the current `overlayVisible`.
...
4. **Security/privacy:** localhost websocket only; authentication on. The password is stored in the dock's local storage and embedded as a query parameter in the overlay URL that the dock's Diagnostics view generates (the static `setup.html` and the auto-created Browser Source use the password-free URL form) (the overlay must authenticate too but has no input UI; that URL lives in OBS's scene collection on the same local disk where OBS itself stores the websocket password in plaintext — same trust domain, verified in Phase 0). All operator text escaped; no internet-loaded assets; no telemetry.
...
14. Show overlay while the mapped source is active in Program → blocking warning (once per session, "don't ask again this session"); with Studio Mode off and source activity unknown → warning also fires; the state chip reads LIVE whenever `videoActive`/`obsSourceActiveChanged` reports the source active and the content is render-shown (render-hidden + source-active reads HIDDEN with a "source is live in Program" detail, per §8.11 chip precedence). A bridge-hotkey Show bypasses the modal and is logged.
...
29. With the overlay Browser Source configured at the wrong size, FPS, or with "Shutdown source when not visible" checked, the connected diagnostic names each mismatched setting with its expected value; with matching settings it reads ok. The source's eye state is shown read-only when available: a disabled (off) eye shows a note explaining the source is hidden in OBS; an enabled eye, or an eye state that isn't known, shows nothing — there is no separate affirmative "eye is on" line, matching the settings-mismatch check's own silence when everything already matches.
30. With the overlay URL present in two sources, the diagnostic shows the multiple-source warning naming both; with one source, no warning.
31. Typing a label character outside the bundled fonts' Latin repertoire (e.g. `→` or `あ`) shows a non-blocking glyph-coverage warning in Setup naming the character; plain Latin text shows none; the warning never blocks Save or Start.
32. Opening `dist/setup.html` by double-clicking it in a normal browser shows the correct absolute `file://` dock URL and password-free overlay URL for its location, each with a working copy control, plus the recommended Browser Source settings — with no network access.
